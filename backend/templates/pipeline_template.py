...
def calculate_iv(df, target):
    ...
    for col in df.columns:
        ...
        try:
            ...
            stats['bad'] = stats['sum']; stats['good'] = stats['count'] - stats['sum']
            total_good = stats['good'].sum(); total_bad = stats['bad'].sum()
...
def run_pipeline():
    ...
    importance = pd.DataFrame({'feature': train_x.columns, 'importance': getattr(model, 'feature_importances_', np.zeros(len(train_x.columns)))})
